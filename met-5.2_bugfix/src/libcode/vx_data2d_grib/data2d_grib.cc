...
#ifdef _WINDOWS
bool MetGrib1DataFile::open(const wchar_t * _filename)
#else
...
#ifdef _WINDOWS
  wprintf(L"Unable to opening file %s\n", _filename);
#else
  printf("Unable to opening file %s\n", _filename);
#endif
  close();
  return ( false );
...
int MetGrib1DataFile::data_plane_array(VarInfo &vinfo,
   ...
   for(i=0; i<GF->n_records(); i++) {
      ...
      if(is_range_match(*vinfo_grib, r)) {
         ...
         ///cerr << Debug(3) << "MetGrib1DataFile::data_plane_array() -> "
...
void MetGrib1DataFile::rotate_winds(VarInfoGrib &vinfo_grib, DataPlane &plane) {
   ...
   if(vinfo_grib.is_u_wind()) {
      ...
      ///cerr << Debug(3) << "MetGrib1DataFile::rotate_winds() -> "
...
bool MetGrib1DataFile::data_plane_scalar(VarInfoGrib &vinfo_grib,
   ...
   for(i=0; i<GF->n_records(); i++) {
      ...
      if(is_exact_match(vinfo_grib, r)) {
         ...
         ///cerr << Debug(3) << "MetGrib1DataFile::data_plane_scalar() -> "
